defmodule ClawdEx.AI.Providers.Groq do
  ...
  Groq 使用 OpenAI 兼容 API，提供超快速推理。

  API 文档: https://console.groq.com/docs/api-reference

  特性:
  - OpenAI 兼容的 chat/completions 端点
  - 支持流式和非流式响应（SSE）
  - 超低延迟推理
  - 模型格式: groq/llama-3.3-70b-versatile → 提取 llama-3.3-70b-versatile

  配置:
  - GROQ_API_KEY 环境变量
  - 或 Application.get_env(:clawd_ex, :groq)[:api_key]
  """
  ...
  defp do_chat(model, messages, api_key, opts) do
    ...
    case Req.post("#{@base_url}/chat/completions",
      ...
      {:ok, %{status: status, body: body}} ->
        Logger.error("Groq API error: status=#{status}, body=#{inspect(body)}")
    ...
    case Req.request(request) do
      {:ok, response} ->
        if response.status >= 200 and response.status < 300 do
          ...
          Logger.error("Groq stream error: HTTP #{response.status}")
          {:error, {:api_error, response.status, inspect(response.body)}}
        end

      {:error, reason} ->
        Logger.error("Groq stream request failed: #{inspect(reason)}")
        {:error, reason}
    end
  end
  ...
  # OpenAI-compatible event processing (same as OpenRouter)
  defp process_event(%{"choices" => [%{"delta" => delta} | _]} = event, acc, stream_to) do
    ...
    # 处理 usage (Groq includes x_groq usage in stream)
  ...
  defp parse_response(resp) do
    Logger.warning("Unexpected Groq response format: #{inspect(resp)}")
